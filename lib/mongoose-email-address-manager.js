...
module.exports = function (schema, options) {
  ...
  function getEmailAddresses(_this) {
    return _this[s('email_addresses')] || [];
  }
  //_mongoose.Schema
  schema.add(_defineProperty({}, s('email_addresses'), [new schema.constructor((_ref = {}, _defineProperty(_ref, s('email_address'), {
    type: String,
    required: true
  ...
  if (index) {
    var emlFIndexOpts = {};
    if (sparse) {
      emlFIndexOpts.sparse = true;
    }
    if (unique) {
      emlFIndexOpts.unique = true;
    }
    schema.index(_defineProperty({}, emailAddressKey, 1), emlFIndexOpts);
  ...
  schema.pre('save', function (next) {
    if (!this.email && getEmailAddresses(this).length > 0) {
      getEmailAddresses(this)[0][s('primary')] = new Date();
    }
  ...
  schema.methods.addEmail = function (emailToAdd) {
    getEmailAddresses(this).push(isObject(emailToAdd) ? emailToAdd : _defineProperty({}, s('email_address'), emailToAdd));
  ...
  schema.methods.getEmail = function (emailToFind) {
    return getEmailAddresses(this).find(function (email) {
  ...
  schema.methods.removeEmail = function (emailToRemove) {
    ...
    if (email) {
      this[s('email_addresses')] = getEmailAddresses(this).filter(function (em) {
        return em !== email;
      });

      if (getEmailAddresses(this).length && !this.email) {
        this.setPrimaryEmail(getEmailAddresses(this)[0]);
      }
    }

    return getEmailAddresses(this);
  };
      ...
      return regeneratorRuntime.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            ...
            case 3:
              return _context4.abrupt('return', getEmailAddresses(this));
